...
donnees_comparaison <- df_graphique %>%
  select(
    mean_pv,
    digital_games_usage_for_learning, 
    digital_resources_usage_for_learning_at_school_by_day,
  ...
  filter(!is.na(Frequence), !is.na(mean_pv))

synthese_graphique <- donnees_comparaison %>%
  group_by(Type_Activite, Frequence) %>%
  summarise(Note_Moyenne = mean(mean_pv, na.rm = TRUE), .groups = "drop")
...
ggplot(synthese_graphique, aes(x = Frequence, y = Note_Moyenne, color = Type_Activite, group = Type_Activite)) +
  ...
  labs(
    title = "Effet de l'utilisation de la technologie sur la performance scolaire",
    x = "Intensité d'utilisation (1 = Min, 9 = Max)",
    y = "Score Moyen PISA",
    color = "Activité"
  )
```
