...
1.  Les variables liées aux technologies et à leurs utilisations.

Ce groupe de variables permet de quantifier la fréquence, le contexte (école ou domicile) et le but (apprentissage ou loisir) de l'utilisation des outils numériques par les élèves.

ST326Q01JA (digital_resources_usage_for_learning_at_school_by_day) : Mesure l'utilisation quotidienne de ressources numériques pour l'apprentissage à l'école.

ST326Q02JA (digital_resources_usage_for_learning_outside_school_by_day) : Mesure l'utilisation quotidienne de ressources numériques pour l'apprentissage en dehors de l'école.

ST326Q03JA (digital_resources_usage_for_learning_on_weekends) : Mesure l'utilisation de ressources numériques pour l'apprentissage pendant le week-end.

ST326Q04JA (digital_resources_usage_for_leisure_at_school_by_day) : Mesure l'utilisation quotidienne de ressources numériques pour les loisirs à l'école.

ST326Q05JA (digital_resources_usage_for_leisure_outside_school_by_day) : Mesure l'utilisation quotidienne de ressources numériques pour les loisirs en dehors de l'école.

ST326Q06JA (digital_resources_usage_for_leisure_on_weekends) : Mesure l'utilisation de ressources numériques pour les loisirs pendant le week-end.

ST253Q01JA (nb_digital_devices_at_home) : Quantifie le nombre d'appareils numériques disponibles au domicile de l'élève.

IC174Q10JA (digital_games_usage_for_learning) : Évalue l'utilisation de jeux numériques à des fins éducatives.

2.  Les variables liées aux performances éducatives.

Ces variables servent à évaluer la réussite et le parcours scolaire des élèves évalués par l'OCDE.

PV1MATH (pv_math) : Le score de l'élève dans le domaine de la culture mathématique.

PV1READ (pv_reading_comprehension) : Le score de l'élève en compréhension de l'écrit (lecture).

PV1SCIE (pv_science) : Le score de l'élève dans le domaine de la culture scientifique.

REPEAT (repeated_a_year) : Indique si l'élève a redoublé une année au cours de sa scolarité .

AJOUTER analyse des NA, ...

## 0.2 L'accès matériel : une fracture résorbée ?

Avant de corréler l'usage du numérique aux performances académiques, il convient de dresser un état des lieux de l'accès matériel aux outils technologiques. L'analyse descriptive de la base de données PISA révèle une démocratisation de l'équipement de base. En observant la disponibilité d'une connexion internet, d'un ordinateur pour le travail et d'applications éducatives, on constate qu'une vaste majorité d'élèves dispose ces outils à domicile. Ce niveau d'équipement primaire confirme notre postulat initial : les phénomènes de « fracture numérique » ne caractérisent plus seulement des zones géographiques privées de réseau. L'enjeu de l'inégalité se déplace vers la manière dont ce matériel est exploité par les élèves.
...
Si la fracture d'accès primaire se réduit, l'indice de statut économique, social et culturel (ESCS) demeure un puissant déterminant du niveau d'équipement technologique. L'analyse de la dispersion du nombre d'appareils numériques au sein du foyer selon les quartiles socio-économiques montre une corrélation : les familles les plus favorisées cumulent un nombre significativement plus élevé d'écrans. Cette abondance matérielle dans les milieux aisés confirme que l'enjeu ne réside plus dans le manque d'outils, mais potentiellement dans la surexposition. C'est cette dynamique environnementale, dictée par le capital socio-culturel, qui rend nécessaire la recherche du point de bascule étudié dans la partie suivante (@fig-escs-equipement).

```{r}
#| label: fig-escs-equipement
#| fig-cap: "Nombre d'appareils numériques au domicile selon le quartile socio-économique (ESCS)"
...
responses_escs <- responses %>%
  ...
  mutate(
    ...
    ESCS_quartile = factor(ESCS_quartile, labels = c("Q1 (Plus modeste)", "Q2", "Q3", "Q4 (Plus favorisé)"))
  )

ggplot(responses_escs, aes(x = ESCS_quartile, y = nb_digital_devices_at_home, fill = ESCS_quartile)) +
  geom_boxplot(alpha = 0.7, show.legend = FALSE, color = "darkgray", outlier.alpha = 0.3) +
  scale_fill_brewer(palette = "Blues") +
  labs(
    title = "Volume d'équipement numérique selon le milieu social",
    x = "Indice socio-économique (quartiles ESCS)",
    y = "Nombre d'appareils numériques à la maison"
  ) +
  theme_minimal()
```
...
ggplot(df_barres, aes(x = factor(Modalite), fill = Sexe)) +
  ...
  labs(
    ...
    fill = "Genre",
    caption = "Figure 3 | Source : Données PISA"
  ) +
...
ggplot(synthese_niveau, aes(
  ...
  labs(
    title = "Évolution des scores PISA selon l'usage numérique et la classe",
...
ggplot(synthese_graphique_mixte, aes(x = Frequence, y = Note_Moyenne, color = Type_Activite, group = Type_Activite)) +
  ...
  labs(
    title = "Comparaison de l'impact des jeux éducatifs et des loisirs numériques",
    subtitle = "Moyenne des scores PISA (Maths, Sciences, Lecture) selon l'intensité d'utilisation",
    x = "Fréquence d'utilisation (Échelle normalisée de 1 à 9)",
    y = "Score Moyen PISA",
    color = "Type d'activité numérique",
    caption = "Source : Figure 5 | Données PISA | Note : L'usage des jeux est distingué par un trait discontinu."
  ) +
...
res.mca <- MCA(donnees_acm, graph = FALSE)

fviz_mca_var(res.mca, 
             repel = TRUE, 
             col.var = "cos2", 
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             title = "Profils de genre et équipement numérique") +
  theme_minimal()
```
